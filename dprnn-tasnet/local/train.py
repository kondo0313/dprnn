...
from criterion.sdr import NegSISDR,VolumeAwareNegSISDR
...
#########################################################
parser.add_argument('--alpha', type=float, default=1.0, help='Alpha value for volume-aware SISDR')
...
def main(args):
    ...
    if args.criterion == 'sisdr':
        #criterion = NegSISDR()
        criterion = VolumeAwareNegSISDR(alpha=args.alpha)
    else:
